...
export const ShootingStars = ({ minSpeed = 10, maxSpeed = 30, minDelay = 1200, maxDelay = 4200, starColor = "#9E00FF", trailColor = "#2EB9DF", starWidth = 10, starHeight = 1, className, }) => {
        ...
        const moveStar = () => {
            if (star) {
                setStar((prevStar) => {
                    if (!prevStar)
                        return null;
                    const newX = prevStar.x +
                        prevStar.speed * Math.cos((prevStar.angle * Math.PI) / 180);
                    const newY = prevStar.y +
    ...
    return (_jsxs("svg", { ref: svgRef, className: cn("w-full h-full absolute inset-0", className), children: [star && (_jsx("rect", { x: star.x, y: star.y, width: starWidth * star.scale, height: starHeight, fill: "url(#gradient)", transform: `rotate(${star.angle}, ${star.x + (starWidth * star.scale) / 2}, ${star.y + starHeight / 2})` }, star.id)), _jsx("defs", { children: _jsxs("linearGradient", { id: "gradient", x1: "0%", y1: "0%", x2: "100%", y2: "100%", children: [_jsx("stop", { offset: "0%", style: { stopColor: trailColor, stopOpacity: 0 } }), _jsx("stop", { offset: "100%", style: { stopColor: starColor, stopOpacity: 1 } })] }) })] }));
};
