...
const Skeleton = ({ className, ...props }) => {
    return (_jsx("div", { className: cn("rounded-md bg-primary/10", className), ...props }));
};
...
export const TweetBody = ({ tweet }) => (_jsx("div", { className: "break-words leading-normal tracking-tighter", children: tweet.entities.map((entity, idx) => {
        switch (entity.type) {
            case "url":
            case "symbol":
            case "hashtag":
...
export const MagicTweet = ({ tweet, components, className, ...props }) => {
    ...
    return (_jsxs("div", { className: cn("relative flex size-full max-w-lg bg-white dark:bg-transparent flex-col gap-2 overflow-hidden rounded-lg border p-4 backdrop-blur-md", className), ...props, children: [_jsx(TweetHeader, { tweet: enrichedTweet }), _jsx(TweetBody, { tweet: enrichedTweet }), _jsx(TweetMedia, { tweet: enrichedTweet })] }));
};
/**
